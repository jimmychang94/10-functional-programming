...
(function(module) {
  ...
  Article.numWordsAll = () => {
    return Article.all.map(numWords => numWords.body.length).reduce((a, b) => a + b);
  };
  ...
  Article.allAuthors = () => {
    const authArr = [];
    return Article.all.map( uniAuthor => uniAuthor.author)
      .reduce((acc, curr) => {
        if (!authArr.includes(curr)) {
          authArr.push(curr);
        }
        return authArr;
      });
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      return {
        authorName: author,
        numWords: Article.all.filter(authNum => authNum.author === author).map(authNum => authNum.body.length).reduce((acc, curr) => acc + curr)
      }
    });
